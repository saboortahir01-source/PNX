...
export function cannedReply(raw: string): string | null {
  ...
  if (/(who (made|built|created)|who'?s (the )?(founder|creator)|about (the )?founder|saboor)/.test(t)) {
    return "![Saboor Tahir — Founder of PNX](/saboor-tahir.png)\n\nPNX was built by **Saboor Tahir** — an independent founder and SEO strategist.\n\nThe idea was simple: pro-grade SEO shouldn't cost $99 a month. So PNX is free, with no signup wall, no daily caps and no upsells. It runs live page audits, keyword research, SERP analysis and YouTube SEO — all from this one chat.\n\nWant to see it in action? Paste a URL and I'll audit it right now.";
  }
  if (/(is (it|pnx) free|pricing|how much (does )?(it|pnx) cost|do i (have to )?pay|subscription)/.test(t)) {
    return "PNX is **100% free** — no signup wall, no daily limits, no credit card, no upsells.\n\nYou get live page audits, keyword research, competitor and SERP analysis, YouTube SEO and content planning. Your chats stay in your own browser's storage, so nothing's locked behind an account.\n\nWhat would you like to run first?";
  }
