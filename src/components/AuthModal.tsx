    ...
    try {
      ...
      if (error) {
        if (error.message.includes("provider is not enabled") || error.message.includes("Unsupported provider")) {
          toast.error("Azure / Microsoft provider configuration required", {
            description: "Turn on Azure in your Supabase Auth settings to sign in with Azure.",
          });
        } else {
          toast.error("Azure sign-in failed", { description: error.message });
        }
      }
    } catch (err: any) {
